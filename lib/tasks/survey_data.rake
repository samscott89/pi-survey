...
namespace :db do
  ...
  task populate_survey: :environment do
	...
	simple_date = OptionChoice.create!(choice_name: "SimpleDate", option_group: g_date)
  	...
  	survey = Survey.create!(name: "Example Survey", description: "This is an example survey.")
  	
  	sec = SurveySection.create!(name: "Section 1", title: "Introduction", required: true, survey: survey, index: 1)
	...
	q = Question.create!(survey_section: sec, name: "Select", subtext: "Select an option", option_group: g_select)
  	...
  	sec = SurveySection.create!(name: "Section 2", title: "Random Things", required: false, survey: survey, index: 2)
  	5.times do 
